...
/**
  ...
  */
class Student(studentName: String = "No name set yet", var registrationNumber: Int = 0) extends Person(studentName) {

  def reset(name: String, registrationNumber: Int): Unit = {
    this.name = name
  ...
  def equals(otherStudent: Student): Boolean = hasTheSameName(otherStudent) && registrationNumber.equals(otherStudent.registrationNumber)
