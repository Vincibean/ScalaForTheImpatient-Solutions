...
class Bundle extends Item {
  ...
  var itemList: List[Item] = List.empty[Item]

  override def price: Double = itemList.map(_.price).sum

  override def description: String = itemList.map(_.description).mkString("; ")
